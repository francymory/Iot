...
def check_isolato(braccialetto_id):
    ...
    for altro_braccialetto in altri_braccialetti:
        ...
        if distanza <= 1000:  
            return jsonify({"isolato": 0}), 200 #se c'è un altro braccialetto nel raggio d'azione allora non è isolato

    return jsonify({"isolato": 1}), 200 #altrimenti è isolato
